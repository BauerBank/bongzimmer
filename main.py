...
def print_out(to_print, body=""):
    ...
    to_print_chunks = [
        to_print[i : i + CHUNK_SIZE] for i in range(0, len(to_print), CHUNK_SIZE)
    ]
    ...
    for update in updates:
        if update.message is not None and (update.message.message_id > LAST_UPDATE_ID):
            ...
            if update.message.chat.username in allowed_chat_ids:
                if (
                    update.message.text[-5:].lower() == "start"
                    or update.message.text[-4:].lower() == "help"
                    or update.message.text[-4:].lower() == "test"
                ):
                    reply_text = (
                        "Hello "
                ...
                await bot.sendMessage(
                    chat_id=update.message.chat.id, text="Sorry, that's not allowed."
                )
...
async def fetch_emails():
    ...
    for num in messages[0].split():
        ...
        to_print = (
            ...
            + "\n"
            + "Date: "
            + date_time_obj.strftime("%Y-%m-%d %H:%M:%S %Z")
            + "\n\n"
        )
...
async def main():
    ...
    # Check if updates list is not empty
    if updates:
